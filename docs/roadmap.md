...
- **[S-07](plans/S-07-window-events.md) / P2**：可选 Window 组件及 Win32/XCB Window、统一事件、
  原生值查询和 Renderer 集成已实现；XCB 等待 Linux CI 复测，下一步推进 Wayland 与 Input。
- **[S-08](plans/S-08-third-party-integrations.md) / P2**：规划独立的 SDL3 与 ImGui 可选集成目标；
  第三方依赖不进入 Renderer、Window 或基础安装 component。

## 近期执行顺序

1. 复测 S-07C XCB 后端并推进 S-07D Wayland Window；远端 Linux 矩阵在 Actions 账户额度恢复后补跑。
