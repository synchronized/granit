...
CMake 在构建测试时使用 `granit_asset_tool`，把
...
SPIR-V 或 WebGPU WGSL 载荷。

参数名先通过 `granit_material_parameter_id` 转换为稳定 ID，再与类型、数据和尺寸一起提交。
材质归档字节只需在创建调用期间有效；Shader Library 归档字节及句柄需保持到 Material 销毁，
因为 Pipeline 按需创建。格式和更新规则见 [Material 参考](../reference/material.md)。
...
回读是 GPU 集成测试的自动验证步骤，不是 Render Pipeline 的必需阶段。实际应用可以继续采样输出
Texture、传递给后处理，或改用 Swapchain Frame 进行窗口显示。
