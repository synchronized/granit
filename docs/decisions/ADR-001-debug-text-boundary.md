...
- 状态：已接受
- 日期：2026-08-14
- 后续修订：文字整形与栅格化职责由 [ADR-002](ADR-002-text-input-boundary.md) 部分取代

## 背景

H-06 已建立 Unlit、Canvas Draw List、动态几何上传和 Tone Mapping 后扩展点。线框、Gizmo、文字与
...
- 公共 Canvas Draw List、Debug Draw 和 Text C ABI 必须分别设计；内部原型不能直接视为稳定公共接口。
