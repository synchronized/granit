...
Upload Batch 将多次 Buffer 写入合并为一次 Vulkan Queue 提交和一次同步等待。它适合初始化大量
DEVICE/AUTOMATIC Buffer，或在一帧中集中上传多个小数据块。

当前实现是 P-04B 的第一阶段，只支持 Buffer；Texture 批量写入尚未实现。API 和 ABI 仍处于早期
开发阶段，不保证兼容性。
...
- Batch 会保活已记录的目标 Buffer。记录后销毁公开 Buffer 句柄不会使尚未提交的上传悬空。
