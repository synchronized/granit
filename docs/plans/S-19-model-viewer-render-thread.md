...
设计已确认，S-19A 实现中。首阶段只修改示例私有代码，不增加公共 API，也不改变浏览器主循环。
...
1. **S-19A：不可变帧包**。移除 Render Desc 对 Core/GPU Scene 可变数组的借用，并验证移动后描述
   仍指向帧包自身。
2. **S-19B：执行器边界**。增加私有 `frame_executor` 与同步实现，桌面和浏览器先保持原行为。
3. **S-19C：UI 与资源槽**。把 ImGui 输出复制为纯 CPU 数据，并为每个在途帧准备独立 Canvas。
4. **S-19D：桌面线程执行器**。增加有界队列、完成回执、Resize/恢复命令和确定性关闭顺序。
