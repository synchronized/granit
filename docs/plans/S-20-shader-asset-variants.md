...
**实现中。** 首阶段直接替换尚未发布的旧容器，不保留旧格式读取或迁移代码。
...
1. **S-20A 清单与 sidecar**：拆分确定性资产，补齐摘要、损坏、缺失和缓存恢复测试。
2. **S-20B 变体选择契约**：显式记录后端、能力档位、入口点和阶段，并提供只读解析接口。
3. **S-20C 打包裁剪**：让工具按 Vulkan、WebGPU 或全量目标导出，缺失目标变体时失败。
4. **S-20D 可选源码前端**：先评估 HLSL，再评估 GLSL；验证绑定、坐标系和能力降级后才接入。
