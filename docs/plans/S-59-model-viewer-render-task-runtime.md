...
**实施中，P1。** 本计划延续 S-57 的运行时拆分和 S-58 的统一资产入口，把只接收帧与无类型命令的
`frame_executor` 提升为强类型渲染任务执行器，并在其上建立 Desktop/Web 共用的 Viewer 生命周期。
...
1. **S-59A 计划与契约基线（进行中）**：登记任务类型、所有权、背压、完成结果和平台边界。
2. **S-59B 强类型执行器**：重命名执行器与测试，使用拥有型任务替换裸回调命令，保持现有帧替换和
   同步等待行为。
3. **S-59C Desktop 迁移**：将上传、Pipeline/质量修改、Swapchain 重建与资源释放迁移到类型化任务，
   删除渲染服务中的回调桥接。
4. **S-59D Web 迁移**：持久持有 inline 执行器，通过相同任务协议完成上传、帧和释放，不再每帧
   临时构造执行器。
