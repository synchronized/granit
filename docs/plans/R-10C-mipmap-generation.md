...
已完成 C ABI、C++20 包装、范围与能力校验、逐级线性 Blit 和资源保活。
...
已覆盖 7×5 非二次幂、Cube 独立数组层、非零起始 mip、范围越界、缺少传输用途、单 mip
Texture、跨 Renderer 句柄和真实末级像素回读。当前资源模型尚未开放 2D Array，数组层测试
使用已支持的 Cube 六层模型，不在本任务中扩大 Texture 类型范围。
