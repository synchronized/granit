...
4. S-02D（已完成）：Device Lost 状态记录首个触发操作，并通过 `device` 类别输出包含
   Granit 结果、Vulkan 后端信息、验证状态和 Renderer domain 的一次性报告。
5. S-02E（已完成）：补齐 C11/C++20、并发回调、回调异常边界、资源失效、跨 Renderer、
   纯管理句柄和未启用 Debug Utils 的降级测试及参考文档。
...
S-02 已完成统一诊断出口、公共回调、GPU 调试名称和首次 Device Lost 报告。回调不会由 Granit
内部串行化；调用方负责线程安全，且不得重入产生消息的同一 Renderer。更完整的日志系统、设备
故障扩展数据和持久化对象名称仍属于后续增强，不影响本任务验收。
