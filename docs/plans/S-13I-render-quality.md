...
实现中。4× MSAA/Resolve、PBR Specular AA、Mipmap、各向异性过滤和 FXAA 基础路径已经存在；
设备能力回退、公开质量开关、查看器控制和跨后端组合验收尚未完成。
...
Render Pipeline 创建描述在尾部追加两个布尔策略字段，并保留零值语义。新初始化宏提供当前推荐
默认值；旧结构大小继续采用兼容默认行为。自定义录制回调维持单采样契约，除非未来单独扩展回调
描述以显式声明多采样支持。
...
- UI 不经过 FXAA；单采样、自定义回调和旧 ABI 调用保持兼容。
...
- 质量开关可能增加 Shader/Pipeline 变体数量，需要复用现有材质缓存并记录首次创建成本。
