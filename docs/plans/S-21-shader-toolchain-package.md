...
ShaderTools 可使用系统 DXC 和 Tint，但官方 CI 与 Release 需要一份可重复下载、
...
- Windows 与 Linux 分别发布宿主工具包，包含 DXC、Tint 及必要运行库和许可证。
...
1. 已建立跨平台清单生成与验证脚本，记录完整文件集合、角色、大小和 SHA-256，并拒绝篡改及
   未登记文件。
2. 已建立原子组包脚本，强制分别提供 DXC、Dawn/Tint 许可证材料，并显式收集运行库。
3. 已建立确定性的源码许可证汇总器，并将锁定 DXC 与 Dawn/Tint 的许可证、第三方声明
   和必要运行库纳入归档。
...
- 两个平台从空缓存下载后均能完成 WGSL、HLSL 双后端资产测试。
...
- 工具升级时必须重新核对 DXC 与 Dawn/Tint 的许可证、第三方声明和运行库闭包。
