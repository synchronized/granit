...
**已确认，尚未实施。** S-20 已完成本地工具发现、兼容策略、能力探测和二进制身份；本任务只负责
官方工具产物的构建、发布和消费闭环。
...
1. 核对 DXC、glslang 与 Dawn/Tint 的再分发许可和运行库闭包。
2. 建立手动触发的 Windows/Linux 构建、能力验证与归档工作流。
3. 发布预发行工具包，记录归档 SHA-256，并用干净环境验证下载和解包。
4. 为 CI 增加锁定下载辅助脚本，设置统一工具链根目录与 Tint 修订号。
5. 在 ShaderTools 完整测试通过后，将官方 CI 切换为 `locked`。
