...
- 设计状态：已确认
- 实现状态：实现已完成；远端 CI 验收受账户额度限制，待恢复后重跑
- 路线图任务：S-03
- 优先级：P1
- 前置依赖：S-01、S-02
...
5. S-03E（已完成）：安装指南已补齐验证矩阵、版本/component 检查、导出审计、DLL 部署和
   常见故障排查；本地 Windows 共享/静态验收通过，远端跨平台重跑等待 Actions 恢复。
...
- 2026-08-16 的 Windows/Linux Actions Job 因账户付款失败或额度限制在启动前被平台拒绝；没有
  代码失败日志。恢复额度后必须重跑两条工作流，再把远端验收状态改为通过。

## 完成结论

S-03 的安装包契约、版本选择测试、导出审计、四类 Consumer 和 Windows/Linux CI 配置均已实现。
本地 Windows MSVC 共享与静态安装结果已经通过完整 Consumer 运行；远端矩阵属于外部状态阻塞，
在实际成功运行前不将其描述为已经通过。
