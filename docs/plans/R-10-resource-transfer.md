...
1. R-10A1：格式 Footprint 与纯 CPU 验证。
2. R-10A2：同步 Texture Readback C ABI、C++ 包装和像素测试。
3. R-10A3：新增原始截图示例与使用指南。
4. R-10B：根据 Render Graph 和资产上传实际需求补齐显式 GPU 复制。
5. R-10C：实现受能力门禁的 mipmap 生成。
6. R-10D：基于测量决定是否增加异步读取抽象。
