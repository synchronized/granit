...
2. R-10B2：根据资产上传调用方需求决定是否公开 Buffer-to-Texture Recorder 命令。
...
5. R-10B2～B3：根据资产上传和批量需求补齐剩余复制入口。
6. R-10C：实现受能力门禁的 mipmap 生成。
7. R-10D：基于测量决定是否增加异步读取抽象。
