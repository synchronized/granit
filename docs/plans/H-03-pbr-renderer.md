...
1. **H-03A（进行中）**：H-03A1 已完成内存模型、语义校验和真实 Pipeline 创建；H-03A2a 已
   完成二进制 v2、往返及损坏输入检查，H-03A2b 将接入源 JSON 和调试 JSON。
...
读取器明确拒绝旧主版本，不增加迁移分支。源 JSON fixture 已切换版本号，具体状态字段与可读调试
输出在 H-03A2b 完成。
