...
> 本文件保存 H-02 的原始设计和逐阶段实施历史。当前边界与完成摘要以
> [H-02 权威计划](../plans/H-02-material-system.md)为准；不要继续在本记录追加未来计划。
...
5. **H-02E（已完成）**：H-02E1 已完成内存版本化材质包和稳定变体查找，H-02E2 已接入 Shader、
   ...
   [独立计划](../plans/H-02-material-package-format.md)。按 D-09A 显式记录绑定模型和 Renderer 能力要求，
   但首版只接受传统 Bind Group。
