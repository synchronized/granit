...
# P-06/H-01：Render Graph 设计与实施记录

> 本文件保存 P-06 架构评估与 H-01A～H-01E 实施历史。当前职责边界和完成摘要以
> [P-06 权威计划](../plans/P-06-render-graph-boundary.md)为准；不要继续在本记录追加未来计划。
...
5. **H-01E：性能复核（已完成）**——已测量编译、录制、提交及资源创建成本，并记录优化门槛。
...
该接口仍仅由独立测试目标构建，使用 `std::function` 也不会跨动态库 ABI。瞬态资源继续复用
现有公开创建描述和底层资源生命周期，不形成第二套 GPU 资源实现。

## H-01C 实现记录

H-01C 支持用现有 `granit_buffer_desc` 和 `granit_texture_desc` 声明瞬态资源。瞬态 Texture 自动
创建完整默认 View，Pass 通过同一逻辑资源 ID 解析该 View；图层不向回调暴露父 Texture 句柄。

编译器的资源生命周期决定实际创建和回收位置：资源在第一个保留 Pass 录制前创建，在最后一个
使用它的 Pass 录制后按 View、Texture/Buffer 顺序立即撤销公开句柄。Recorder 已记录的命令持有
内部强引用，因此资源对象仍安全存活到 GPU 提交完成；使用者无法在图执行后继续访问瞬态句柄。
...
Windows Clang Release 首份基线显示，100 Pass 编译 P50 为 57.030 µs，1000 Pass 为
525.798 µs；100 Pass 图执行相对直接 Recorder 增加约 69.470 µs。当前编译成本近似线性且未达到
P-05 的并行化重评门槛，因此暂不引入图缓存或外部执行器。

100 个独立瞬态 Buffer 的图执行 P50 为 1.740 ms，相对同规模导入资源图增加约 1.480 ms。该数据
证明资源创建与销毁是下一优化候选，但尚不能证明需要内存别名。真实重复帧若稳定创建约
64～100 个同描述资源，且该路径超过 1 ms 或帧 CPU 的 10%，应先验证按描述复用的瞬态资源池。

完整环境、数据和复测条件见
[Render Graph 首份性能基线](../../benchmarks/results/2026-08-11-windows-clang-render-graph-6bac5a5.md)。
