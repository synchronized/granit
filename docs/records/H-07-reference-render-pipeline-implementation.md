...
> 本文件保存 H-07 的原始设计、公共 ABI 收敛过程和逐阶段实施历史。当前设计与剩余任务以
> [H-07 权威计划](../plans/H-07-reference-render-pipeline.md)为准；不要继续在本记录追加未来计划。
...
4. **H-07H 验收（进行中）**：自动离屏路径已覆盖 8 轮 Pipeline 创建、真实渲染和销毁，确认默认
   ...
   与手工 H-05 的 0.662 ms 处于同一量级。约 78.5% 的自动路径时间已归因到 Shadow/Opaque Draw，
   而不是门面外壳。H-07H 暂不通过，后续先缓存每 Draw 重复创建的常量 Buffer、Bind Group 和
   光照资源，再建立等工作量复测。完整数据见
