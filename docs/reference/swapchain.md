...
查询函数声明在 `<granit/renderer/frame_context.h>`，不要求调用方持有 Swapchain 句柄。

```c
granit_frame_info frame_info = GRANIT_FRAME_INFO_INIT;
granit_result result = granit_frame_get_info(renderer, frame, &frame_info);
```
...
`GRANIT_ERROR_INVALID_HANDLE`。Renderer 与 Frame 不属于同一对象关系时也返回该错误。调用者必须
使用 `GRANIT_FRAME_INFO_INIT`，预留字段必须为零。
