...
本文描述当前公开 API 的线程安全边界。C++ RAII 包装内部会修改自身句柄，因此同一个拥有对象不能
并发调用 `initialize`、`reset`、`destroy`、`map` 或其他成员函数。需要并行录制时，应在同步点后
复制对应的非拥有 `*_ref`，并保证原包装对象在工作线程结束前存活。只有 C 回调或显式互操作边界
需要复制 `native_handle()`。
...
Renderer 诊断回调在产生消息的线程同步执行，并可能由多个线程并发调用。回调接收器必须线程
安全，且不得重入产生消息的同一 Renderer；完整生命周期规则见 [Renderer](renderer.md)。

Win32 窗口句柄的线程亲和、消息循环和销毁顺序仍由调用方遵守平台规则；Granit 的内部锁不会
放宽操作系统对窗口对象的要求。
