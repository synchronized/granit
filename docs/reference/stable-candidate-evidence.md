...
| 公共头等级检查 | 新安装头遗漏长期候选、观察候选或实验性分类 | `granit.contracts.api_stability` |
...
Clang 和 GCC。公共头编译测试继续覆盖所有版本尺寸宏，0.36.0 的 component 快照额外固定关键拥有
型、借用型和逐帧结构的数值布局。
...
|---|---|---|---|
| Core | 长期候选范围已明确 | 0.29.0 跨版本符号基线、0.36.0 精确快照、资源与句柄行为、C11/C++20 Consumer | 在首个稳定版本前继续验证弃用流程 |
| Window | 长期候选范围已明确 | 0.25.0 跨版本符号基线、0.36.0 精确快照、多窗口与 Surface 重建 Consumer | 在更多宿主环境中继续验证平台生命周期 |
| RenderPipeline | 保持观察候选 | 0.29.0 跨版本符号基线、0.36.0 精确快照、资产/场景/渲染行为与 Consumer | 继续观察真实上游对 Material、Scene 与同步 render 门面的演进需求 |
