...
Window 是独立可选 component，CMake 使用者目标为 `granit::window`。当前已实现 Win32 Window
System、顶层窗口、轮询事件和原生句柄查询；XCB 与 Wayland Window 运行时仍在计划中。
...
队列为空返回 `GRANIT_ERROR_NOT_READY`。当前 Win32 后端产生：
