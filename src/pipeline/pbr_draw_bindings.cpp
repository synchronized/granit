...
granit_result pbr_draw_bindings::initialize(
    granit_renderer renderer, const material_draw_state& material,
  ...
  if (renderer == GRANIT_NULL_HANDLE || initialized() ||
      material.frame_layout == GRANIT_NULL_HANDLE ||
  ...
  const std::array frame_entry{granit::bind_group_entry{
      .binding = 0,
      .resource = frame_buffer_.native_handle(),
      .offset = 0,
      .size = sizeof(frame)}};
  if (granit::succeeded(result))
