  ...
  *output = GRANIT_NULL_HANDLE;
  ...
  try {
    ...
    if (system->screen == nullptr)
      return GRANIT_ERROR_BACKEND_UNAVAILABLE;
    system->wm_protocols = intern_xcb_atom(system->connection, "WM_PROTOCOLS");
    system->wm_delete_window = intern_xcb_atom(system->connection, "WM_DELETE_WINDOW");
    system->wm_size_hints = intern_xcb_atom(system->connection, "WM_SIZE_HINTS");
    system->net_wm_name = intern_xcb_atom(system->connection, "_NET_WM_NAME");
    system->utf8_string = intern_xcb_atom(system->connection, "UTF8_STRING");
    system->owner_thread = std::this_thread::get_id();
