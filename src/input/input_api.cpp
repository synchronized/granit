...
#if defined(_WIN32)
  std::unordered_map<granit_window, wchar_t> pending_high_surrogates;
#endif
};
...
#if defined(_WIN32)
  input.pending_high_surrogates.erase(window);
#endif
...
void clear_focus(void* user_data, granit_window window) {
  ...
  auto& pointer = input.pointers[window];
  if (pointer.struct_size == 0)
    pointer = GRANIT_POINTER_STATE_INIT;
  pointer.buttons = 0;
#if defined(_WIN32)
  input.pending_high_surrogates.erase(window);
#endif
...
void emit_text(input_record& input, granit_window window, const wchar_t* text, int length) {
...
void native_event(void* user_data, granit_window window, std::uint32_t message, std::uintptr_t word,
  ...
  if (message == WM_KEYDOWN || message == WM_SYSKEYDOWN || message == WM_KEYUP ||
    ...
    auto& state = keyboard_for(input, window);
    ...
    enqueue(input, window, GRANIT_INPUT_EVENT_KEY, data);
    return;
  }
  if (message == WM_CHAR) {
    const auto code_unit = static_cast<wchar_t>(word);
    if (code_unit >= 0xd800 && code_unit <= 0xdbff) {
      input.pending_high_surrogates[window] = code_unit;
      return;
    }
    if (code_unit >= 0xdc00 && code_unit <= 0xdfff) {
      const auto found = input.pending_high_surrogates.find(window);
      if (found != input.pending_high_surrogates.end()) {
        const wchar_t pair[]{found->second, code_unit};
        input.pending_high_surrogates.erase(found);
        emit_text(input, window, pair, 2);
      }
      return;
    }
    input.pending_high_surrogates.erase(window);
    emit_text(input, window, &code_unit, 1);
    return;
  }
  if (message == WM_UNICHAR && word != UNICODE_NOCHAR && word <= 0x10ffff) {
    ...
    if (word <= 0xffff) {
      ...
      const auto value32 = static_cast<std::uint32_t>(word - 0x10000);
      text[0] = static_cast<wchar_t>(0xd800 + (value32 >> 10));
      text[1] = static_cast<wchar_t>(0xdc00 + (value32 & 0x3ff));
      length = 2;
    }
    emit_text(input, window, text, length);
    return;
  }

  const auto x = static_cast<float>(GET_X_LPARAM(value));
  const auto y = static_cast<float>(GET_Y_LPARAM(value));
  auto& state = pointer_for(input, window);
  if (message == WM_MOUSEMOVE) {
    ...
    if (state.inside == 0)
      enqueue(input, window, GRANIT_INPUT_EVENT_POINTER_ENTERED, data);
    state.x = x;
    state.y = y;
    state.buttons = data.pointer_moved.buttons;
    state.inside = 1;
    enqueue(input, window, GRANIT_INPUT_EVENT_POINTER_MOVED, data);
    return;
  }
  if (message == WM_MOUSELEAVE) {
    ...
    enqueue(input, window, GRANIT_INPUT_EVENT_POINTER_LEFT, data);
    return;
  }
  ...
  if (button != 0) {
    ...
    enqueue(input, window, GRANIT_INPUT_EVENT_POINTER_BUTTON, data);
    return;
  }
  if (message == WM_MOUSEWHEEL || message == WM_MOUSEHWHEEL) {
    ...
    enqueue(input, window, GRANIT_INPUT_EVENT_POINTER_WHEEL, data);
  }
}
#else
