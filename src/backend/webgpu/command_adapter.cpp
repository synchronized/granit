...
public:
  ...
  ~webgpu_command_recorder_resource() override {
    if (command_buffer_ != 0) {
      static_cast<void>(
          context_->backend->destroy_command_buffer(context_->instance, command_buffer_));
    }
    if (recorder_ != 0) {
      static_cast<void>(context_->backend->destroy_command_recorder(context_->instance, recorder_));
    }
  }
...
granit_result end_compute_if_open(const webgpu_command_context& context,
  ...
  const auto result = context.backend->recorder_end_compute(context.instance, recorder.recorder_);
...
webgpu_command_adapter::webgpu_command_adapter(webgpu_context& context,
                                               webgpu_instance_handle instance)
    : context_(
          std::make_shared<webgpu_command_context>(webgpu_command_context{&context, instance})) {}
...
webgpu_command_adapter::begin(backend_command_recorder_resource& resource) const noexcept {
  ...
  return context_->backend->create_command_recorder(context_->instance, &recorder->recorder_);
}
...
granit_result webgpu_command_adapter::begin_rendering(
  ...
  if (const auto result = end_compute_if_open(*context_, *recorder); result != GRANIT_SUCCESS)
    return result;
  auto result = context_->backend->recorder_begin_rendering(
  ...
  if (recorder->pipeline_ != 0) {
    replay(context_->backend->recorder_bind_pipeline(context_->instance, recorder->recorder_,
                                                     recorder->pipeline_));
  }
  for (const auto& binding : recorder->groups_) {
    replay(context_->backend->recorder_bind_graphics_groups(
  ...
  for (const auto& binding : recorder->vertex_buffers_) {
    replay(context_->backend->recorder_bind_vertex_buffers(context_->instance, recorder->recorder_,
                                                           binding.first, binding.bindings));
  }
  if (recorder->index_buffer_ != 0) {
    replay(context_->backend->recorder_bind_index_buffer(
  ...
  if (!recorder->viewports_.empty()) {
    replay(context_->backend->recorder_set_viewports(
        context_->instance, recorder->recorder_, recorder->viewport_first_, recorder->viewports_));
  }
  if (!recorder->scissors_.empty()) {
    replay(context_->backend->recorder_set_scissors(context_->instance, recorder->recorder_,
                                                    recorder->scissor_first_, recorder->scissors_));
  }
  if (result != GRANIT_SUCCESS) {
    static_cast<void>(
        context_->backend->recorder_end_rendering(context_->instance, recorder->recorder_));
...
granit_result
webgpu_command_adapter::bind_pipeline(backend_command_recorder_resource& resource,
  ...
  if (recorder->render_open_) {
    const auto result = context_->backend->recorder_bind_pipeline(context_->instance,
...
granit_result webgpu_command_adapter::bind_graphics_groups(
  ...
  try {
    ...
    if (recorder->render_open_) {
      const auto result = context_->backend->recorder_bind_graphics_groups(
...
webgpu_command_adapter::begin_compute(backend_command_recorder_resource& resource) const noexcept {
  ...
  if (recorder == nullptr || recorder->recorder_ == 0 || recorder->command_buffer_ != 0 ||
    ...
    return GRANIT_ERROR_INVALID_ARGUMENT;
  const auto result =
      context_->backend->recorder_begin_compute(context_->instance, recorder->recorder_);
  if (result == GRANIT_SUCCESS)
...
granit_result
webgpu_command_adapter::bind_compute_pipeline(backend_command_recorder_resource& resource,
                                              webgpu_compute_pipeline pipeline) const noexcept {
  auto* recorder = as_recorder(resource);
  if (recorder == nullptr)
    return GRANIT_ERROR_INVALID_ARGUMENT;
  if (!recorder->compute_open_) {
  ...
  return context_->backend->recorder_bind_compute_pipeline(context_->instance, recorder->recorder_,
                                                           pipeline);
}
...
granit_result webgpu_command_adapter::bind_compute_groups(
  ...
  return recorder == nullptr ? GRANIT_ERROR_INVALID_ARGUMENT
                             : context_->backend->recorder_bind_compute_groups(
...
granit_result webgpu_command_adapter::dispatch(backend_command_recorder_resource& resource,
  ...
  auto* recorder = as_recorder(resource);
  return recorder == nullptr ? GRANIT_ERROR_INVALID_ARGUMENT
                             : context_->backend->recorder_dispatch(context_->instance,
...
granit_result webgpu_command_adapter::bind_vertex_buffers(
  ...
  try {
    ...
    if (recorder->render_open_) {
      const auto result = context_->backend->recorder_bind_vertex_buffers(
...
granit_result webgpu_command_adapter::bind_index_buffer(backend_command_recorder_resource& resource,
  ...
  if (recorder == nullptr || recorder->recorder_ == 0 || recorder->command_buffer_ != 0 ||
    ...
    return GRANIT_ERROR_INVALID_ARGUMENT;
  if (recorder->render_open_) {
    const auto result = context_->backend->recorder_bind_index_buffer(
...
granit_result
webgpu_command_adapter::set_viewports(backend_command_recorder_resource& resource,
                                      std::uint32_t first,
  ...
  try {
    ...
    if (recorder->render_open_) {
      const auto result = context_->backend->recorder_set_viewports(
...
granit_result
webgpu_command_adapter::set_scissors(backend_command_recorder_resource& resource,
                                     std::uint32_t first,
  ...
  try {
    ...
    if (recorder->render_open_) {
      const auto result = context_->backend->recorder_set_scissors(
...
granit_result webgpu_command_adapter::copy_texture_to_buffer(
    ...
    std::uint32_t width, std::uint32_t height, std::uint32_t bytes_per_row) const noexcept {
  auto* recorder = as_recorder(resource);
  if (recorder == nullptr || recorder->render_open_ || texture == 0 || buffer == 0)
    return GRANIT_ERROR_INVALID_ARGUMENT;
  if (const auto result = end_compute_if_open(*context_, *recorder); result != GRANIT_SUCCESS)
    return result;
  return context_->backend->recorder_copy_texture_to_buffer(
...
granit_result webgpu_command_adapter::copy_buffer(
  ...
  if (const auto result = end_compute_if_open(*context_, *recorder); result != GRANIT_SUCCESS)
    return result;
  return context_->backend->recorder_copy_buffer(context_->instance, recorder->recorder_, source,
                                                 destination, regions);
}
...
granit_result webgpu_command_adapter::copy_buffer_to_texture(
  ...
  if (const auto result = end_compute_if_open(*context_, *recorder); result != GRANIT_SUCCESS)
    return result;
  return context_->backend->recorder_copy_buffer_to_texture_v2(
...
granit_result webgpu_command_adapter::copy_texture_to_buffer(
  ...
  if (const auto result = end_compute_if_open(*context_, *recorder); result != GRANIT_SUCCESS)
    return result;
  return context_->backend->recorder_copy_texture_to_buffer_v2(
      context_->instance, recorder->recorder_, source, destination, region);
}

granit_result
webgpu_command_adapter::copy_texture(backend_command_recorder_resource& resource,
  ...
  if (const auto result = end_compute_if_open(*context_, *recorder); result != GRANIT_SUCCESS)
    return result;
  return context_->backend->recorder_copy_texture(context_->instance, recorder->recorder_, source,
                                                  destination, region);
}
...
granit_result webgpu_command_adapter::fill_buffer(backend_command_recorder_resource& resource,
  ...
  if (const auto result = end_compute_if_open(*context_, *recorder); result != GRANIT_SUCCESS)
    return result;
  return context_->backend->recorder_fill_buffer(context_->instance, recorder->recorder_, buffer,
                                                 offset, size, value);
}

granit_result
webgpu_command_adapter::generate_mipmaps(backend_command_recorder_resource& resource,
  ...
  if (const auto result = end_compute_if_open(*context_, *recorder); result != GRANIT_SUCCESS)
    return result;
  return context_->backend->recorder_generate_mipmaps(context_->instance, recorder->recorder_,
                                                      texture, range);
}
...
granit_result webgpu_command_adapter::draw(backend_command_recorder_resource& resource,
  ...
  if (recorder == nullptr || recorder->recorder_ == 0 || recorder->command_buffer_ != 0)
    return GRANIT_ERROR_INVALID_ARGUMENT;
  return context_->backend->recorder_draw_vertices(context_->instance, recorder->recorder_,
...
granit_result webgpu_command_adapter::draw_indexed(backend_command_recorder_resource& resource,
  ...
  if (recorder == nullptr || recorder->recorder_ == 0 || recorder->command_buffer_ != 0)
    return GRANIT_ERROR_INVALID_ARGUMENT;
  return context_->backend->recorder_draw_indices(context_->instance, recorder->recorder_,
...
webgpu_command_adapter::end(backend_command_recorder_resource& resource) const noexcept {
  ...
  if (recorder->compute_open_) {
    const auto result =
        context_->backend->recorder_end_compute(context_->instance, recorder->recorder_);
    if (result != GRANIT_SUCCESS)
      return result;
    recorder->compute_open_ = false;
  }
  return context_->backend->finish_command_recorder(context_->instance, recorder->recorder_,
...
webgpu_command_adapter::submit(backend_command_recorder_resource& resource) const noexcept {
  ...
  if (recorder == nullptr || recorder->command_buffer_ == 0)
    return GRANIT_ERROR_INVALID_ARGUMENT;
  const auto result =
      context_->backend->submit_command_buffer(context_->instance, recorder->command_buffer_);
  if (result == GRANIT_SUCCESS)
...
webgpu_command_adapter::reset(backend_command_recorder_resource& resource) const noexcept {
  ...
  if (recorder->command_buffer_ != 0) {
    const auto result =
        context_->backend->destroy_command_buffer(context_->instance, recorder->command_buffer_);
    if (result != GRANIT_SUCCESS)
      return result;
    recorder->command_buffer_ = 0;
  }
  if (recorder->recorder_ != 0) {
    const auto result =
        context_->backend->destroy_command_recorder(context_->instance, recorder->recorder_);
    if (result != GRANIT_SUCCESS)
      return result;
...
webgpu_command_recorder webgpu_command_adapter::native_recorder(
    backend_command_recorder_resource& resource) const noexcept {
  const auto* recorder = as_recorder(resource);
