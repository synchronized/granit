...
granit_result vulkan_readback_context::initialize(const vulkan_device& device) noexcept {
...
granit_result vulkan_readback_context::ensure_capacity(vulkan_memory_allocator& allocator,
  ...
  const auto result = allocator.create_buffer(info, vulkan_memory_location::readback, replacement);
  if (result != GRANIT_SUCCESS)
    return result;
...
granit_result vulkan_readback_context::begin(const vulkan_device& device) noexcept {
...
granit_result vulkan_readback_context::end(const vulkan_device& device) noexcept {
  return map_vulkan_result(device.functions().vkEndCommandBuffer(command_buffer_));
}
granit_result vulkan_readback_context::reset_fence(const vulkan_device& device) noexcept {
  return fence_.reset(device);
}
granit_result vulkan_readback_context::wait(const vulkan_device& device,
                                            std::uint64_t timeout) noexcept {
  return fence_.wait(device, timeout);
}
granit_result
vulkan_readback_context::restore_signaled_fence(const vulkan_device& device) noexcept {
  fence_.destroy(device);
  return fence_.initialize(device, true);
}
void vulkan_readback_context::destroy(const vulkan_device& device,
