...
material_runtime_template::create(granit_renderer renderer, material_package package,
  ...
  try {
    ...
    const auto result =
        candidate->gpu_.initialize(renderer, candidate->package_, {}, shader_library);
...
material_reload_result material_hot_reload_slot::reload(granit_renderer renderer,
  ...
  const auto result =
      material_runtime_template::create(renderer, std::move(package), candidate, shader_library);
