...
bool valid_index_entry_name(std::string_view value) {
  const auto separator = value.find('/');
  if (separator == std::string_view::npos)
    return logical_name(value);
  return value.find('/', separator + 1) == std::string_view::npos &&
         logical_name(value.substr(0, separator)) && identifier(value.substr(separator + 1));
}
...
bool parse_hex(std::string_view text, std::span<std::byte> bytes) {
  ...
  for (std::size_t index = 0; index < bytes.size(); ++index) {
    const auto high = value(text[index * 2]);
    const auto low = value(text[index * 2 + 1]);
    if (high < 0 || low < 0)
      return false;
...
shader_library_source_error encode_shader_library_index_json(const shader_library_index& index,
                                                             std::string& json) noexcept {
  try {
    if (!identifier(index.library) || index.shaders.empty() ||
        index.shaders.size() > maximum_shader_count)
      return shader_library_source_error::invalid_argument;
    std::vector<const shader_library_index_entry*> entries;
    entries.reserve(index.shaders.size());
    for (const auto& shader : index.shaders) {
      if (!valid_index_entry_name(shader.name) || !identifier(shader.entry_point) ||
          stage_name(shader.stage).empty())
        return shader_library_source_error::invalid_argument;
      entries.push_back(&shader);
    }
    std::ranges::sort(entries, {}, [](const auto* entry) { return entry->name; });
    for (std::size_t index_value = 1; index_value < entries.size(); ++index_value) {
      if (entries[index_value - 1]->name == entries[index_value]->name)
        return shader_library_source_error::invalid_argument;
    }

    std::ostringstream output;
    output << "{\n  \"format_version\": 1,\n  \"library\": \"" << index.library
           << "\",\n  \"content_digest\": \"" << hex(index.library_digest)
           << "\",\n  \"shaders\": [\n";
    for (std::size_t index_value = 0; index_value < entries.size(); ++index_value) {
      const auto& shader = *entries[index_value];
      output << "    {\"name\": \"" << shader.name << "\", \"content_id\": \""
             << hex(shader.content_id) << "\", \"stage\": \"" << stage_name(shader.stage)
             << "\", \"entry_point\": \"" << shader.entry_point << "\"}";
      output << (index_value + 1 == entries.size() ? "\n" : ",\n");
    }
    output << "  ]\n}\n";
    json = output.str();
    return shader_library_source_error::none;
...
shader_library_source_error parse_shader_library_index_json(std::string_view json,
                                                            shader_library_index& index) noexcept {
  if (json.empty() || json.size() > maximum_json_size)
    return shader_library_source_error::invalid_argument;
  try {
    json_value root;
    ...
    const auto* object = as<json_value::object>(&root);
    if (object == nullptr ||
        !only_members(*object, {"format_version", "library", "content_digest", "shaders"}))
      return shader_library_source_error::invalid_schema;
    const auto* version = as<std::uint64_t>(member(*object, "format_version"));
    if (version == nullptr)
      return shader_library_source_error::invalid_schema;
    if (*version != 1)
      return shader_library_source_error::unsupported_version;
    const auto* library = as<std::string>(member(*object, "library"));
    const auto* digest = as<std::string>(member(*object, "content_digest"));
    const auto* shaders = as<json_value::array>(member(*object, "shaders"));
    shader_library_index replacement;
    if (library == nullptr || !identifier(*library) || digest == nullptr || shaders == nullptr ||
        shaders->empty() || shaders->size() > maximum_shader_count ||
        !parse_hex(*digest, replacement.library_digest))
      return shader_library_source_error::invalid_schema;
    replacement.library = *library;
    std::set<std::string, std::less<>> names;
    for (const auto& value : *shaders) {
      const auto* shader = as<json_value::object>(&value);
      if (shader == nullptr ||
          !only_members(*shader, {"name", "content_id", "stage", "entry_point"}))
        return shader_library_source_error::invalid_schema;
      const auto* name = as<std::string>(member(*shader, "name"));
      const auto* content_id = as<std::string>(member(*shader, "content_id"));
      const auto* stage = as<std::string>(member(*shader, "stage"));
      const auto* entry_point = as<std::string>(member(*shader, "entry_point"));
      shader_library_index_entry entry;
      if (name == nullptr || !valid_index_entry_name(*name) || content_id == nullptr ||
          stage == nullptr || entry_point == nullptr || !identifier(*entry_point) ||
          !parse_stage(*stage, entry.stage) || !parse_hex(*content_id, entry.content_id) ||
          !names.emplace(*name).second)
        return shader_library_source_error::invalid_schema;
      entry.name = *name;
      entry.entry_point = *entry_point;
      replacement.shaders.push_back(std::move(entry));
    }
    index = std::move(replacement);
    return shader_library_source_error::none;
  } catch (const std::bad_alloc&) {
    return shader_library_source_error::out_of_memory;
  } catch (...) {
