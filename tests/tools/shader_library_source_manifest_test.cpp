...
TEST_CASE("Shader Library 源清单解析并规范化 Define") {
  granit::tools::shader_library_source_manifest manifest;
  REQUIRE(granit::tools::parse_shader_library_source_manifest(valid_manifest, manifest) ==
...
TEST_CASE("Shader Library 源清单拒绝无效语法和版本") {
  granit::tools::shader_library_source_manifest manifest;
  CHECK(granit::tools::parse_shader_library_source_manifest("{", manifest) ==
        granit::tools::shader_library_source_error::invalid_json);
  const auto unsupported =
      replace_once(std::string{valid_manifest}, "\"format_version\": 1", "\"format_version\": 2");
  CHECK(granit::tools::parse_shader_library_source_manifest(unsupported, manifest) ==
        granit::tools::shader_library_source_error::unsupported_version);
}
...
TEST_CASE("Shader Library 源清单严格校验作者输入") {
  ...
  for (const auto& json : invalid_manifests) {
    granit::tools::shader_library_source_manifest manifest;
    CHECK(granit::tools::parse_shader_library_source_manifest(json, manifest) ==
          granit::tools::shader_library_source_error::invalid_schema);
  }
}

TEST_CASE("Shader Library 索引编码按逻辑名称确定排序") {
  granit::tools::shader_library_index first{.library = "pbr_standard",
                                            .library_digest = {},
  ...
  REQUIRE(granit::tools::encode_shader_library_index_json(first, first_json) ==
          granit::tools::shader_library_source_error::none);
  REQUIRE(granit::tools::encode_shader_library_index_json(second, second_json) ==
          granit::tools::shader_library_source_error::none);
...
TEST_CASE("Shader Library 索引拒绝重复逻辑名称") {
  ...
  std::string json;
  CHECK(granit::tools::encode_shader_library_index_json(index, json) ==
        granit::tools::shader_library_source_error::invalid_argument);
}
