...
int main(int argc, char** argv) {
  ...
  granit::shader_tools::source_library_desc desc{
  ...
  auto [status, cache_hit] = granit::shader_tools::build_library_from_manifest(desc);
  if (status.failed() || cache_hit || !std::filesystem::exists(library) ||
      !std::filesystem::exists(index))
    return 2;
  std::tie(status, cache_hit) = granit::shader_tools::build_library_from_manifest(desc);
