...
int main(int argc, char** argv) {
  ...
  const std::string toolchain_root = argv[1];
  const std::string input = argv[2];
  granit::asset_tools::shader::compiler compiler;
  if (compiler.initialize({toolchain_root}).failed())
    return 3;
  const granit::asset_tools::shader::shader_define definitions[]{
      {.name = "TEST_VALUE", .value = "1"}};
  ...
  const granit::asset_tools::shader::shader_define duplicate_definitions[]{definitions[0],
                                                                           definitions[0]};
