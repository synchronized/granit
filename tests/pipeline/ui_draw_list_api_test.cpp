...
constexpr std::array vertices{granit_ui_vertex{0, 0, 0, 0, UINT32_C(0xffffffff)},
                              granit_ui_vertex{1, 0, 1, 0, UINT32_C(0xffffffff)},
                              granit_ui_vertex{0, 1, 0, 1, UINT32_C(0xffffffff)}};
constexpr std::array<uint32_t, 3> indices{0, 1, 2};

} // namespace

TEST_CASE("公共UI Draw List支持通用几何、矩形、合批与复用") {
  ...
  granit_ui_draw_list_desc desc = GRANIT_UI_DRAW_LIST_DESC_INIT;
  desc.initial_vertex_capacity = 16;
  desc.initial_index_capacity = 24;
  desc.initial_item_capacity = 4;
  granit::ui_draw_list list;
  REQUIRE(list.initialize(renderer.native_handle(), desc) == granit::result::success);

  const granit_ui_draw_state first{11, 21, {0, 0, 128, 128}, 0};
  const granit_ui_draw_state second{12, 21, {0, 0, 128, 128}, 0};
  REQUIRE(list.append(vertices, indices, first) == granit::result::success);
  REQUIRE(list.append(vertices, indices, first) == granit::result::success);

  granit_ui_rect_desc rect = GRANIT_UI_RECT_DESC_INIT;
  rect.x = 10;
  rect.y = 20;
  rect.width = 30;
  rect.height = 40;
  ...
  granit_ui_draw_list_stats stats = GRANIT_UI_DRAW_LIST_STATS_INIT;
  ...
  stats = GRANIT_UI_DRAW_LIST_STATS_INIT;
  REQUIRE(list.get_stats(stats) == granit::result::success);
...
TEST_CASE("公共UI Draw List拒绝无效输入、跨Renderer与旧句柄") {
  ...
  granit_ui_draw_list list = GRANIT_NULL_HANDLE;
  granit_ui_draw_list_desc desc = GRANIT_UI_DRAW_LIST_DESC_INIT;
  REQUIRE(granit_ui_draw_list_create(first.native_handle(), &desc, &list) == GRANIT_SUCCESS);
  const granit_ui_draw_state state{11, 21, {}, 0};
  CHECK(granit_ui_draw_list_append(second.native_handle(), list, vertices.data(), 3, indices.data(),
                                   3, &state) == GRANIT_ERROR_INVALID_HANDLE);
  CHECK(granit_ui_draw_list_append(first.native_handle(), list, nullptr, 3, indices.data(), 3,
                                   &state) == GRANIT_ERROR_INVALID_ARGUMENT);
  const std::array<uint32_t, 2> non_triangles{0, 1};
  CHECK(granit_ui_draw_list_append(first.native_handle(), list, vertices.data(), 3,
                                   non_triangles.data(), 2,
                                   &state) == GRANIT_ERROR_INVALID_ARGUMENT);
  auto invalid_vertices = vertices;
  invalid_vertices[0].x = std::numeric_limits<float>::infinity();
  CHECK(granit_ui_draw_list_append(first.native_handle(), list, invalid_vertices.data(), 3,
                                   indices.data(), 3, &state) == GRANIT_ERROR_INVALID_ARGUMENT);
  granit_ui_rect_desc rect = GRANIT_UI_RECT_DESC_INIT;
  rect.state = state;
  CHECK(granit_ui_draw_list_append_rect(first.native_handle(), list, &rect) ==
        GRANIT_ERROR_INVALID_ARGUMENT);

  REQUIRE(granit_ui_draw_list_destroy(first.native_handle(), list) == GRANIT_SUCCESS);
  CHECK(granit_ui_draw_list_destroy(first.native_handle(), list) == GRANIT_ERROR_INVALID_HANDLE);
  granit_ui_draw_list_stats stats = GRANIT_UI_DRAW_LIST_STATS_INIT;
  CHECK(granit_ui_draw_list_get_stats(first.native_handle(), list, &stats) ==
