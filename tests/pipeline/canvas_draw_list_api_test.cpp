...
constexpr std::array vertices{granit::canvas_vertex{0, 0, 0, 0, UINT32_C(0xffffffff)},
                              granit::canvas_vertex{1, 0, 1, 0, UINT32_C(0xffffffff)},
                              granit::canvas_vertex{0, 1, 0, 1, UINT32_C(0xffffffff)}};
...
TEST_CASE("公共Canvas Draw List支持通用几何、矩形、合批与复用") {
  ...
  REQUIRE(list.initialize(renderer, {.initial_vertex_capacity = 16,
                                     .initial_index_capacity = 24,
                                     .initial_item_capacity = 4,
                                     .frame_slot_count = GRANIT_DEFAULT_FRAMES_IN_FLIGHT}) ==
          granit::result::success);
  CHECK(list.ref().native_handle() == list.native_handle());

  const granit::canvas_draw_state first{.texture = granit::texture_view_ref::from_native(11),
                                        .sampler = granit::sampler_ref::from_native(21),
                                        .clip = {0, 0, 128, 128}};
  const granit::canvas_draw_state second{.texture = granit::texture_view_ref::from_native(12),
                                         .sampler = granit::sampler_ref::from_native(21),
                                         .clip = {0, 0, 128, 128}};
...
TEST_CASE("公共Canvas Draw List支持一次追加共享几何的多个范围") {
  ...
  constexpr std::array batch_vertices{granit::canvas_vertex{0, 0, 0, 0, UINT32_C(0xffffffff)},
                                      granit::canvas_vertex{1, 0, 1, 0, UINT32_C(0xffffffff)},
                                      granit::canvas_vertex{1, 1, 1, 1, UINT32_C(0xffffffff)},
                                      granit::canvas_vertex{0, 1, 0, 1, UINT32_C(0xffffffff)}};
  constexpr std::array<uint32_t, 6> batch_indices{0, 1, 2, 0, 2, 3};
  constexpr granit::canvas_draw_state state{.texture = granit::texture_view_ref::from_native(11),
                                            .sampler = granit::sampler_ref::from_native(21),
                                            .clip = {0, 0, 128, 128}};
  constexpr std::array ranges{granit::canvas_draw_range{0, 3, state},
                              granit::canvas_draw_range{3, 3, state}};
