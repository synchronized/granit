...
TEST_CASE("Surface创建把空Renderer归类为无效句柄", "[surface][contract]") {
  ...
  desc.source.win32 = {reinterpret_cast<void*>(1), reinterpret_cast<void*>(1)};
...
TEST_CASE("Linux Surface 公共入口验证原生描述", "[surface][xcb][wayland]") {
  ...
  CHECK(granit_surface_create(UINT64_C(1), &desc, &output) == GRANIT_ERROR_INVALID_ARGUMENT);
  CHECK(output == GRANIT_NULL_HANDLE);
  desc.source.xcb.connection = &output;
  desc.source.xcb.window = UINT32_C(1);
  CHECK(granit_surface_create(UINT64_C(1), &desc, &output) == GRANIT_ERROR_INVALID_HANDLE);

  desc = GRANIT_SURFACE_DESC_INIT;
  desc.surface_type = GRANIT_SURFACE_TYPE_WAYLAND_BIT;
  CHECK(granit_surface_create(UINT64_C(1), &desc, &output) == GRANIT_ERROR_INVALID_ARGUMENT);
  desc.source.wayland.display = &output;
  desc.source.wayland.surface = &desc;
  CHECK(granit_surface_create(UINT64_C(1), &desc, &output) == GRANIT_ERROR_INVALID_HANDLE);
}

TEST_CASE("Canvas Surface 校验 CSS selector 契约", "[surface][canvas][contract]") {
  granit_surface output = UINT64_C(42);
  CHECK(granit_surface_create(UINT64_C(1), nullptr, &output) == GRANIT_ERROR_INVALID_ARGUMENT);

  granit_surface_desc desc = GRANIT_SURFACE_DESC_INIT;
  CHECK(granit_surface_create(UINT64_C(1), &desc, &output) == GRANIT_ERROR_INVALID_ARGUMENT);
  desc.surface_type = GRANIT_SURFACE_TYPE_WIN32_BIT | GRANIT_SURFACE_TYPE_XCB_BIT;
  CHECK(granit_surface_create(UINT64_C(1), &desc, &output) == GRANIT_ERROR_INVALID_ARGUMENT);
  desc = GRANIT_SURFACE_DESC_INIT;
  ...
  desc = GRANIT_SURFACE_DESC_INIT;
  desc.surface_type = GRANIT_SURFACE_TYPE_CANVAS_BIT;
  desc.source.canvas.selector_length = 1;
  CHECK(granit_surface_create(UINT64_C(1), &desc, &output) == GRANIT_ERROR_INVALID_ARGUMENT);
  desc.source.canvas.selector = "#canvas";
  desc.source.canvas.selector_length = 0;
  CHECK(granit_surface_create(UINT64_C(1), &desc, &output) == GRANIT_ERROR_INVALID_ARGUMENT);
  constexpr char embedded_zero[] = {'#', 'a', '\0', 'b'};
  desc.source.canvas.selector = embedded_zero;
  desc.source.canvas.selector_length = sizeof(embedded_zero);
  CHECK(granit_surface_create(UINT64_C(1), &desc, &output) == GRANIT_ERROR_INVALID_ARGUMENT);
...
TEST_CASE("Renderer 销毁时自动使所属 Surface 失效", "[surface][lifetime]") {
  ...
  surface_desc.source.win32 = {window.instance(), window.window()};
  granit_surface surface = GRANIT_NULL_HANDLE;
  REQUIRE(granit_surface_create(renderer, &surface_desc, &surface) == GRANIT_SUCCESS);
