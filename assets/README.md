...
`generated/installed` 的公开文件安装到 `share/granit/assets`。`generated/embedded` 的内容只用于
构建 Granit 自身，不安装或形成公共契约。
...
- 更新 `.grshlib` 时必须同步更新引用它的 `.grmat`；逻辑名称表已经包含在 Library 内。
