...
5. 默认在完整任务达到可评审状态后统一推送并创建一个 Pull Request；普通 Pull Request 更新不
   自动触发 Actions，合并前按改动范围手动运行 Linux、Windows 或 Documentation。
6. 只有本地无法覆盖的平台差异、远端集成环境或 CI 配置问题，才在任务完成前推送到同一个特性
   分支进行阶段验证；修复失败时继续复用同一 Pull Request。
7. 合并前手动触发的 Actions 通过后再合并，`main` Actions 自动运行一次作为最终确认。用户明确
   要求立即推送、拆分 Pull Request 或阶段性远端验证时，以用户要求为准。
