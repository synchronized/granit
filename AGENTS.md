...
- 修改文档前阅读并遵守根目录 `DOCUMENTATION_GUIDE.md`。
- README、Guide、Reference、Concept、Roadmap、Plan、ADR 和 Record 保持职责分离。
- 同一事实只保留一个权威来源，其他文档使用简短摘要和相对链接。
- 当前行为、未来计划和历史记录不得混写；大段实现日志不得持续追加到计划正文。
- 新增文档进入 `docs/README.md` 的对应分类，不在根 README 堆叠完整文档清单。
...
1. 一个完整路线图任务或可独立评审的功能使用一个特性分支；子阶段在同一分支连续开发，不为每个
   子步骤创建独立分支或 Pull Request。
2. 修改前阅读相关源码、测试、构建文件和文档，确认边界及验证方式，并检查工作区已有修改。
3. 实施最小必要改动，同步补充测试和文档；子阶段完成后可以创建聚焦的本地提交。
4. 开发期间优先运行本地格式检查、相关构建和测试；高风险改动扩大本地验证矩阵。
5. 默认在完整任务达到可评审状态后统一推送并创建一个 Pull Request；Pull Request 更新和合并到
   `main` 均不自动触发普通 CI，合并前按改动范围手动运行 Linux、Windows 或 Documentation。
6. 只有本地无法覆盖的平台差异、远端集成环境或 CI 配置问题，才在任务完成前推送到同一个特性
   分支进行阶段验证；修复失败时继续复用同一 Pull Request。
7. 合并前手动触发的 Actions 通过后再合并；Release、依赖和镜像工作流遵守各自触发策略。用户
   明确要求立即推送、拆分 Pull Request 或阶段性远端验证时，以用户要求为准。
8. 每次提交前运行 `git diff --check` 并检查提交范围；最终总结说明变更内容、验证结果、未验证项
   和剩余风险。
9. 提交、推送或创建 Pull Request 仍须获得用户明确授权；已有授权只在其明确覆盖的任务范围内
   生效。
