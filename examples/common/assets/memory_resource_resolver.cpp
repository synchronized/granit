...
bool memory_resource_resolver::insert(std::string_view resource_uri,
                                      std::span<const std::byte> bytes) {
  std::string normalized;
  if (bytes.empty() || !normalize_resource_path(resource_uri, normalized))
    return false;
...
bool memory_resource_resolver::contains(std::string_view resource_uri) const {
  std::string normalized;
  return normalize_resource_path(resource_uri, normalized) && resources_.contains(normalized);
}

bool memory_resource_resolver::resolve(std::string_view resource_uri,
                                       std::vector<std::byte>& bytes) const {
  std::string normalized;
  if (!normalize_resource_path(resource_uri, normalized))
    return false;
