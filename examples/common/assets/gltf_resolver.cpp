...
gltf_resolver::gltf_resolver(const asset_store& assets, std::string base_path)
    : assets_(assets), base_path_(std::move(base_path)) {
  if (!base_path_.empty() && base_path_.back() != '/')
    base_path_.push_back('/');
}

bool gltf_resolver::resolve(std::string_view path, std::vector<std::byte>& output) const {
