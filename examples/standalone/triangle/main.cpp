...
public:
  granit::result initialize(bool smoke_test) noexcept {
    ...
    shader_archive_ = read_file(GRANIT_STANDALONE_TRIANGLE_SHADER_LIBRARY);
    ...
    result = window_.initialize(window_system_,
                                {.title = "Granit SDK Triangle", .width = 1280, .height = 720});
    if (result.failed())
      return result;
    return renderer_.initialize({.application_name = "Granit SDK Triangle",
