...
#ifndef GRANIT_TUTORIAL_07_SHADER_LIBRARY
#error "GRANIT_TUTORIAL_07_SHADER_LIBRARY must point to the generated Shader Library"
#endif
#ifndef GRANIT_TUTORIAL_07_MATERIAL
#error "GRANIT_TUTORIAL_07_MATERIAL must point to the generated Material archive"
#endif
...
public:
  granit::result initialize(bool smoke_test) noexcept {
    ...
    shader_archive_ = read_file(GRANIT_TUTORIAL_07_SHADER_LIBRARY);
    material_archive_ = read_file(GRANIT_TUTORIAL_07_MATERIAL);
    ...
    result = window_.initialize(window_system_,
                                {.title = "Granit Tutorial 07", .width = 1280, .height = 720});
    if (result.failed())
      return result;
    return renderer_.initialize({.application_name = "Granit Tutorial 07",
...
private:
  granit::result initialize_presentation() noexcept {
    ...
    if (result.ok()) {
      ...
      if (metrics_result != granit::result::unsupported)
  ...
  granit::result initialize_materials() noexcept {
    ...
    if (result.ok()) {
      result = initialize_material(cube_material_, {0.82F, 0.24F, 0.08F, 1.0F}, 0.25F, 0.38F);
    }
    if (result.ok()) {
