...
public:
  granit::result initialize(bool smoke_test) noexcept {
    ...
    shader_archive_ = read_file(GRANIT_TUTORIAL_03_SHADER_LIBRARY);
    if (shader_archive_.empty()) {
      std::cerr << "Failed to read Shader Library: " << GRANIT_TUTORIAL_03_SHADER_LIBRARY << '\n';
    ...
    result = window_.initialize(window_system_,
                                {.title = "Granit Tutorial 03", .width = 1280, .height = 720});
    if (result.failed())
      return result;
    return renderer_.initialize({.application_name = "Granit Tutorial 03",
  ...
  void shutdown(granit::result) noexcept {
    ...
    static_cast<void>(texture_group_.reset());
...
private:
  ...
  granit::result initialize_pipeline_layout() noexcept {
    ...
    auto result = texture_layout_.initialize(renderer_, entries);
    const std::array layout_refs{texture_layout_.ref()};
    if (result.ok())
      result = layout_.initialize(renderer_, layout_refs);

    const std::array resources{
        granit::bind_group_entry{.binding = 0, .resource = checker_view_.ref()},
        granit::bind_group_entry{.binding = 1, .resource = checker_sampler_.ref()},
    };
    if (result.ok())
      result = texture_group_.initialize(renderer_, texture_layout_, resources);
    return result;
  }
  ...
  granit::result render_frame() noexcept {
    ...
    if (result.ok())
      result = recorder.bind_graphics_group(layout_, 0, texture_group_);
    if (result.ok())
      result = recorder.draw(6);
    if (result.ok())
  ...
  granit::bind_group texture_group_;
