...
static_assert(sizeof(raymarch_uniforms) == sizeof(float) * 4);
...
private:
  granit::result on_initialize() noexcept override {
    ...
    if (result.ok())
      result = shader_library_.initialize(renderer(), tutorial_raymarch::shader_archive());
    if (result.ok())
      result = shader_library_.create_shader("raymarch.vertex", vertex_shader_);
    if (result.ok())
      result = shader_library_.create_shader("raymarch.fragment", fragment_shader_);
    granit::renderer_limits limits;
    if (result.ok())
      result = renderer_owner().get_limits(limits);
    if (result.ok()) {
      uniform_stride_ = align_up(sizeof(raymarch_uniforms), limits.uniform_buffer_offset_alignment);
  ...
  granit::result initialize_pipeline_layout() noexcept {
    ...
    const std::array resources{
        granit::bind_group_entry{
            .binding = 0, .resource = uniform_buffer_.ref(), .size = sizeof(raymarch_uniforms)},
    };
    if (result.ok())
  ...
  granit::result on_render(granit::example::present_frame& frame) noexcept override {
    ...
    const raymarch_uniforms uniforms{
...
#if defined(__EMSCRIPTEN__)
extern "C" EMSCRIPTEN_KEEPALIVE std::uint32_t granit_tutorial_04_rendered_frames() noexcept {
  return application.rendered_frames();
}

extern "C" EMSCRIPTEN_KEEPALIVE std::uint32_t granit_tutorial_04_recreate_count() noexcept {
...
int main(int argument_count, char** arguments) {
  ...
  const auto result =
      application.run({.executable_path = executable_path,
                       .title = "Granit Raymarch",
                       .renderer = {.application_name = "Granit Raymarch",
