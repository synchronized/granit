...
granit_result source_error(granit::tools::shader_library_source_error error) {
  using enum granit::tools::shader_library_source_error;
  switch (error) {
  case none:
...
std::string
compile_options(const std::vector<granit::tools::shader_library_source_define>& definitions) {
...
struct expanded_shader {
  std::string name;
  const granit::tools::shader_library_source_shader* shader{};
  const std::vector<granit::tools::shader_library_source_define>* definitions{};
};
  ...
  *cache_hit = 0;
  ...
  try {
    ...
    const auto parse_result =
        granit::tools::parse_shader_library_source_manifest(read_text(manifest_path), manifest);
    if (parse_result != granit::tools::shader_library_source_error::none)
    ...
    const auto encode_result = granit::tools::encode_shader_library_index_json(index, index_json);
    if (encode_result != granit::tools::shader_library_source_error::none)
