...
/** Shader 编译结果句柄。零值无效。 */
typedef uint64_t granit_asset_tools_shader_compilation;
/** Shader Compiler 句柄。零值无效。 */
typedef uint64_t granit_asset_tools_shader_compiler;

/** AssetTools Shader 领域内置目标档位的静态能力；与构建机 GPU 无关。 */
typedef struct granit_asset_tools_shader_target_capabilities {
  uint32_t struct_size;
  granit_shader_backend_flags backend;
  granit_shader_profile profile;
  uint32_t reserved;
  granit_shader_feature_flags supported_features;
} granit_asset_tools_shader_target_capabilities;

#define GRANIT_ASSET_TOOLS_SHADER_TARGET_CAPABILITIES_INIT                                         \
  {(uint32_t)sizeof(granit_asset_tools_shader_target_capabilities), UINT32_C(0),                   \
   GRANIT_SHADER_PROFILE_PORTABLE, UINT32_C(0), UINT64_C(0)}

/** Shader Compiler 配置。工具链根目录在创建调用期间借用。 */
typedef struct granit_asset_tools_shader_compiler_desc {
  uint32_t struct_size;
  uint32_t reserved;
  const char* toolchain_root;
  uint64_t toolchain_root_length;
} granit_asset_tools_shader_compiler_desc;

#define GRANIT_ASSET_TOOLS_SHADER_COMPILER_DESC_INIT                                               \
  {(uint32_t)sizeof(granit_asset_tools_shader_compiler_desc), UINT32_C(0), 0, UINT64_C(0)}

typedef struct granit_asset_tools_shader_define {
  uint32_t struct_size;
  uint32_t reserved;
  const char* name;
...
} granit_asset_tools_shader_define;

#define GRANIT_ASSET_TOOLS_SHADER_DEFINE_INIT                                                      \
  {(uint32_t)sizeof(granit_asset_tools_shader_define), UINT32_C(0), 0, UINT64_C(0), 0, UINT64_C(0)}

/** HLSL 编译描述；spirv_output_path 与 wgsl_output_path 均必填。 */
typedef struct granit_asset_tools_shader_compile_desc {
  uint32_t struct_size;
  ...
  /** 实现按名称排序并拒绝重复项。 */
  const granit_asset_tools_shader_define* defines;
...
} granit_asset_tools_shader_compile_desc;

#define GRANIT_ASSET_TOOLS_SHADER_COMPILE_DESC_INIT                                                \
  {(uint32_t)sizeof(granit_asset_tools_shader_compile_desc),                                       \
...
typedef struct granit_asset_tools_shader_compilation_info {
  uint32_t struct_size;
...
} granit_asset_tools_shader_compilation_info;

#ifdef __cplusplus
extern "C" {
#endif

/** 创建 Compiler；根目录会复制到句柄中，并按固定 bin 布局验证 DXC 与 Tint。 */
 ...
 */
GRANIT_ASSET_TOOLS_API granit_result granit_asset_tools_shader_compiler_compile(
    granit_asset_tools_shader_compiler compiler, const granit_asset_tools_shader_compile_desc* desc,
...
GRANIT_ASSET_TOOLS_API granit_result granit_asset_tools_shader_compilation_get_reflection(
    granit_asset_tools_shader_compilation compilation,
    granit_asset_tools_shader_reflection* reflection);

/** 查询编译生成的 SPIR-V；视图在编译结果销毁前有效。 */
GRANIT_ASSET_TOOLS_API granit_result granit_asset_tools_shader_compilation_get_spirv(
    granit_asset_tools_shader_compilation compilation, const void** data, uint64_t* size);

/** 查询编译生成或规范化的 WGSL；视图在编译结果销毁前有效。 */
GRANIT_ASSET_TOOLS_API granit_result granit_asset_tools_shader_compilation_get_wgsl(
    granit_asset_tools_shader_compilation compilation, const char** source, uint64_t* length);

/** 查询内置目标档位支持的静态特性；当前 backend 使用 ASSET_BACKEND 单值。 */
GRANIT_ASSET_TOOLS_API granit_result granit_asset_tools_shader_get_target_capabilities(
